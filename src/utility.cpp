...
std::pair<std::string, double> Utility::output_one_line(const std::vector<OCRPredictResult> &ocr_result) {
    std::string text("");
    double score = 0;
    for (int i = 0; i < ocr_result.size(); i++) {
    // rec
        if (ocr_result[i].score != -1.0) {
                ...
                score += ocr_result[i].score;
        }
    }
    return std::pair<std::string, double>(text, score);
}
